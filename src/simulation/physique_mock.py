...
class PhysicsMockProvider:
    ...
    def _physics_loop(self):
        ...
        # Ratios : Plus le chiffre est grand, plus le moteur tourne vite à basse vitesse
        ...
        while self._running:
            ...
            # 2. Calcul du RPM (Le lien mécanique)
            if self.gear == 0:
                # Au point mort (N), on fait juste hurler le moteur avec la pédale
                target_rpm = 800 + (self.throttle * 60)
            else:
                # En prise, le régime dépend strictement de la vitesse et de la boîte
                ratio = ratios.get(self.gear, 1)
                target_rpm = (self.speed_kmh * ratio * 10.0) + 800

                # Petit effet de glissement (embrayage) si on accélère très fort à l'arrêt
                if self.throttle > 50 and self.speed_kmh < 15:
                    target_rpm += (self.throttle * 15)

            # Lissage de l'aiguille du compte-tours
            self.rpm += (target_rpm - self.rpm) * 8.0 * dt

            # Limites du moteur (Ralenti et Rupteur)
            if self.rpm < 800: self.rpm = 800
            if self.rpm > 6500: self.rpm = 6500 - (time.time() % 0.1) * 200  # Effet rupteur

            # 3. Simulation de la dynamique des roues
            front_speed = self.speed_kmh
            rear_speed = self.speed_kmh

            if self.throttle > 80 and self.speed_kmh < 50 and self.gear == 1:
                rear_speed = self.speed_kmh + 30.0  # Patinage propulsion
            if self.brake > 80:
                front_speed = 0.0  # Blocage freinage fort

            # 4. Traduction du rapport (Gear) en valeurs brutes CAN pour TripStats
            if self.gear == 1:
                gear_raw = 106
            elif self.gear == 2:
                gear_raw = 109
